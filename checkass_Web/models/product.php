<?php
    ...
    class product_db {
        ...
        static function update($id, $price, $quantity, $name, $content, $img)
        {
            $db = getConnection();
            $result = $db->query(
                "UPDATE product
                SET price = $price, quantity = $quantity, name = '$name', content = '$content', img = '$img'
                WHERE id = $id");
            return $result;
        }

        static function update_noImg($id, $price, $quantity, $name, $content)
        {
            $db = getConnection();
            $result = $db->query(
                "UPDATE product
                SET price = $price, quantity = $quantity, name = '$name', content = '$content'
                WHERE id = $id");
            return $result;
        }
